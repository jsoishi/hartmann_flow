...
def main(filename, start, count, output):
    ...
    # Plot settings
    tasks = ['vx', 'vy', 'Az', 'Bx', 'By']
    scale = 2.5
    dpi = 400
    title_func = lambda sim_time: 't = {:.3f}'.format(sim_time)
    savename_func = lambda write: 'vel_fields_{:06}.png'.format(write)
    # Layout
    nrows, ncols = 3, 2
    ...
    with h5py.File(filename, mode='r') as file:
        for index in range(start, start+count):
            for n, task in enumerate(tasks):
                ...
                plot_tools.plot_bot(dset, image_axes, data_slices, axes=axes, title=task, cmap=current_cmap)#, clim=[-1,1])
            # Add time title
